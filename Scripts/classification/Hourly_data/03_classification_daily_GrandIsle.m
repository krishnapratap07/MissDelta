...
%% Load Grand Isle surge

load GrandIsle_DailyMax_StormSurge.mat

TT_dailys = table2timetable( ...
    out(:,{'Date','DailyMaxStormSurge'}), ...
    'RowTimes','Date');

TT_dailys = TT_dailys(:, 'DailyMaxStormSurge');
TT_dailys.Properties.VariableNames = {'S'};

TT_dailys.Properties.RowTimes.TimeZone = 'America/Chicago';
...
OUTDIR = fullfile(BASE_OUTDIR_5CAT,"daily_with_GrandIsle");
if ~exist(OUTDIR,'dir'); mkdir(OUTDIR); end

save(fullfile(OUTDIR,"TT_dailyall_daily_with_GI.mat"), ...
    "TT_dailyall");

PLOT_TITLE = ...
    'WL P%d events by 5-category driver classification: daily WL with Grand Isle';

LEGEND_NAMES = TT_dailyall.Properties.VariableNames( ...
    ~ismember(TT_dailyall.Properties.VariableNames,{'S','Q'}));

PLOT_FIG_NAME = 'GI_daily_BAR_AllStations_P%d.fig';
PLOT_PNG_NAME = 'GI_daily_BAR_AllStations_P%d.png';

COUNTS_FILE = 'Counts_byStation_byPercentile_GI_daily.mat';
SERIES_FILE = 'Series_byStation_byPercentile_GI_daily.mat';
